...
def rle_decode(mask_rle: str, shape=(256, 256)) -> np.ndarray:
    ...
    if (
        ...
        or not mask_rle.strip()
        or mask_rle.lower() == "nan"
    ):
    ...
    if len(s) % 2 != 0:
        raise ParticipantVisibleError(
            "RLE segmentation must have even-length (start, length) pairs"
        )

    if any(x < 0 for x in s):
        raise ParticipantVisibleError(
            "RLE segmentation must not contain negative values"
        )
...
def dice_coefficient(mask1: np.ndarray, mask2: np.ndarray) -> float:
    ...
    return (2.0 * intersection + 1e-7) / (
        np.sum(mask1) + np.sum(mask2) + 1e-7
...
def score(
    ...
    if not required_columns.issubset(solution.columns) or not required_columns.issubset(
        submission.columns
    ):
        raise ParticipantVisibleError(
            "Solution and submission must contain 'id' and 'segmentation' columns"
        )
    ...
    dice_scores = []
    for solution_seg, submission_seg in zip(
        solution["segmentation"], submission["segmentation"]
    ):
