...
class UNetPlusPlus(nn.Module):
    def __init__(
        ...
        # Replace Upsample with ConvTranspose2d
        self.up1 = nn.ConvTranspose2d(feature_channels[1], feature_channels[1], kernel_size=2, stride=2)
        self.up2 = nn.ConvTranspose2d(feature_channels[2], feature_channels[2], kernel_size=2, stride=2)
        self.up3 = nn.ConvTranspose2d(feature_channels[3], feature_channels[3], kernel_size=2, stride=2)
        self.up4 = nn.ConvTranspose2d(feature_channels[4], feature_channels[4], kernel_size=2, stride=2)
    ...
    def forward(self, x):
        ...
        x0_1 = self.dec0_1(torch.cat([x0_0, self.up1(x1_0)], dim=1))
        x1_1 = self.dec1_1(torch.cat([x1_0, self.up2(x2_0)], dim=1))
        x2_1 = self.dec2_1(torch.cat([x2_0, self.up3(x3_0)], dim=1))
        x3_1 = self.dec3_1(torch.cat([x3_0, self.up4(x4_0)], dim=1))

        x0_2 = self.dec0_2(torch.cat([x0_0, x0_1, self.up1(x1_1)], dim=1))
        x1_2 = self.dec1_2(torch.cat([x1_0, x1_1, self.up2(x2_1)], dim=1))
        x2_2 = self.dec2_2(torch.cat([x2_0, x2_1, self.up3(x3_1)], dim=1))

        x0_3 = self.dec0_3(torch.cat([x0_0, x0_1, x0_2, self.up1(x1_2)], dim=1))
        x1_3 = self.dec1_3(torch.cat([x1_0, x1_1, x1_2, self.up2(x2_2)], dim=1))

        x0_4 = self.dec0_4(torch.cat([x0_0, x0_1, x0_2, x0_3, self.up1(x1_3)], dim=1))
